...
class SQLSHELL(cmd.Cmd):
    ...
    def do_speak_to_the_manager(self, line):
        policy_assignments = []
        ...
        try:
            ...
            guid_result = self.sql_query("select * from dbo.UnknownSystem_DISC")
            for item in guid_result:
                if item['CPUType0'] == 'x64':
                    x64_guid = item['SMS_Unique_Identifier0']
            if not x64_guid:
                print("[-] Could not recover x64 unknown computer GUID")
                sys.exit()
            
            print("[*] Querying for Policy Assignments")
            policies = self.sql_query(f"exec MP_GetMachinePolicyAssignments N'{x64_guid}', N''")
            
            #need to decode each policy body because SCCM hates me and wants me to quit
            ...
            if policies:
                for policy in policies:
                    ...
                    if policy:
                        ...
                        policy_assignments.append((policy_id, policy_version))
            else:
                print("[-] Could not find policy assignments for x64 machine guid")
                
            #now request every policy assignment based on version and id
            print("[*] Querying for policy bodies")
            for policy_id, policy_version in policy_assignments:
